import { getDb } from "./DBManager";

export interface AlbumArtist {
    album_id:number
    artist_id:number
}

export async function getAllArtistsAlbums() {
    const db = await getDb();
    const rows = await db.getAllAsync<AlbumArtist>('SELECT * FROM albums_artists');
    const albumsArtists: AlbumArtist[] = rows.map(row => ({
        album_id:row.album_id,
        artist_id:row.artist_id
    }));
    return albumsArtists;
}

export async function getAlbumArtistById(album_artist:AlbumArtist) {
    const db = await getDb();
    const rows = await db.getAllAsync<{album_artist:AlbumArtist}>(`SELECT * FROM albums_artists WHERE album_id = ? AND artist_id = ?`, [album_artist.album_id, album_artist.artist_id]);
    if (rows.length > 0) return rows; else return null 
}

export async function getArtistsByAlbumId(album_id:number) {
    const db = await getDb();
    const rows = await db.getAllAsync<{ artist_id: number }>(`SELECT artist_id FROM albums_artists WHERE album_id = ?`, [album_id]);
    return rows.map(row => row.artist_id);
}

export async function getAlbumsByArtistId(artist_id:number) {
    const db = await getDb();
    const rows = await db.getAllAsync<{ album_id: number }>(`SELECT album_id FROM albums_artists WHERE artist_id = ?`, [artist_id]);
    return rows.map(row => row.album_id);
}

export async function getAlbumCountById(artist_id:number) {
    const db = await getDb();
    const row = await db.getFirstAsync< {count: number} >(`SELECT COUNT(DISTINCT album_id) AS count FROM albums_artists WHERE artist_id = ?`, [artist_id]);
    if (!row) return 0; else return row 
}

export async function addAlbumArtist(albums_artists:AlbumArtist) {
    const db = await getDb();
    try {
        await db.runAsync(`INSERT INTO albums_artists (album_id, artist_id) VALUES (?, ?)`,
        [albums_artists.album_id, albums_artists.artist_id]
    )
    }
    catch (err) {
        console.error(err)
        throw err
    }
}

export async function deleteArtistsByAlbumId(id:number) {
    const db = await getDb();
    try {
        await db.runAsync(`DELETE FROM albums_artists WHERE album_id = ?`, [id])
    }
    catch (err) {
        console.error(err)
        throw err
    }
}

export async function deleteAlbumsByArtistId(id:number) {
    const db = await getDb();
    try {
    const row = await db.runAsync(`DELETE FROM albums_artists WHERE artist_id = ?`, [id])
    }
    catch (err) {
        console.error(err)
        throw err
    }
}