...
export async function addSongAlbum(songs_albums:SongAlbum) {
    const db = await getDb();
    try {
        await db.runAsync(`INSERT INTO songs_albums (song_id, album_id) VALUES (?, ?)`,
        [songs_albums.song_id, songs_albums.album_id]
    )
    }
    catch (err) {
        console.error(err)
...
export async function deleteSongsByAlbumId(id:number) {
    const db = await getDb();
    try {
        await db.runAsync(`DELETE FROM songs_albums WHERE album_id =?`, [id])
    }
    catch (err) {
        console.error(err)
        throw err
...
export async function deleteAlbumsBySongId(id:number) {
    const db = await getDb();
    try {
        await db.runAsync(`DELETE FROM songs_albums WHERE song_id = ?`, [id])
    }
    catch (err) {
        console.error(err)
        throw err
